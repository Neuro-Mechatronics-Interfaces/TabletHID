...
The current iOS app includes the SwiftUI control surfaces, profile/config persistence, HID descriptor/report byte builders, and an experimental BLE HID transport.

The transport uses `CBPeripheralManager` with the expanded HID service UUID `00001812-0000-1000-8000-00805F9B34FB`. This follows the workaround discussed in developer community testing and should be treated as experimental until it is validated on physical devices and target hosts. The setup screen's Development Mode is still useful for validating UI/report behavior without a host connection.
