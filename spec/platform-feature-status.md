...
| Pause / resume advertising (radio coexistence) | Implemented; advertising is a separate axis from connection. `BleHidManager` exposes public `startAdvertising(target)` / `stopAdvertising()`, `isAdvertising` / `advertisingTarget` StateFlows, and `hasGattServer`; pausing stops the `BluetoothLeAdvertiser` while keeping the GATT server and any live connection intact (Home "Stop Advertising" button), letting a shared BLE receiver pair with another peripheral; resume reuses the open server | TODO |
...
| Community config graph | Server/web implemented with sparse token-similarity edge table, stored dissimilarity score, reconciled persistent cluster tables/labels, visible-node pairwise dissimilarity response, rebuild script, incremental upload/admin-update hooks, and `/configs` Graph tab with collapsible clusters; Android app does not query graph yet | Server/web implemented; iOS app does not query graph yet |
