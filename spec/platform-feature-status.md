...
| Community API client | Implemented (`ConfigApiClient`); `HttpURLConnection`-based; reads `BuildConfig.COMMUNITY_API_BASE_URL`; feature is disabled when URL is empty | Implemented (`ConfigApiClient`); `URLSession`-based `actor`; reads `Bundle.main.infoDictionary["CommunityApiBaseUrl"]` from `Secrets.xcconfig`; feature disabled when URL is empty |
