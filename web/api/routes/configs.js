...
import { getConfigGraph, patchGraphCluster, updateConfigGraph, upsertGraphCluster } from '../graph.js';
...
export async function postConfigGraphCluster(req, res) {
  try {
    const body = req.body ?? {};
    const updated = upsertGraphCluster(db, {
    ...
    if (!updated) return res.status(400).json({ error: 'Invalid cluster payload.' });
    return res.json(updated);
  } catch (err) {
    console.log(JSON.stringify({
      level: 'error',
      event: 'post_graph_cluster_error',
      error: err?.message ?? String(err),
    }));
    return res.status(500).json({ error: 'Internal server error.' });
  }
